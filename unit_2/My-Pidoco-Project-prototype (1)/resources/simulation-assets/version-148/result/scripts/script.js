...
var ajax=rabbit.communication.manager.get(url,_13a,{containerId:"__containerId__",mode:rabbit.util.getMode()},{crossDomain:rabbit.result.manager.isExport});
if(!rabbit.result.manager.isExport){
...
var url,ajax;
if(page){
_1bd.addClass("overlay").addClass("repository").attr("id",id).attr("data-original-layer-id",_1bc).attr("data-page-id",_1bc).appendTo("body").css({width:page.data.width+"px",height:page.data.height+"px"}).data("has-rectangle",false);
...
top=_1ce.top+(_1ca.data.height-parseInt(_1cd))/2;
left=_1ce.left+(_1ca.data.width-parseInt(_1cc))/2;
}
_1cb.show().css({top:top,left:left});
...
$(_2a9.element).css({left:0,top:top});
};
